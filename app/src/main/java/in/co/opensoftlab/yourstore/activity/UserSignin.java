...
public class UserSignin extends BaseActivity implements GoogleApiClient.OnConnectionFailedListener, View.OnClickListener {
            ...
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {
                ...
                if (user != null) {
                    ...
                    if (PrefUtils.getFromPrefs(getApplicationContext(),"loggedState","false").contentEquals("true")){
                        ...
                        startActivity(new Intent(UserSignin.this, LocationFetching.class));
                        finish();
                    }
    ...
    private void signInEmail(String email, String password) {
        ...
        mAuth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        Log.d(TAG_Email, "signInWithEmail:onComplete:" + task.isSuccessful());

                        // If sign in fails, display a message to the user. If sign in succeeds
                        // the auth state listener will be notified and logic to handle the
                        // signed in user can be handled in the listener.
                        if (!task.isSuccessful()) {
                            Log.w(TAG_Email, "signInWithEmail:failed", task.getException());
                            Toast.makeText(UserSignin.this, R.string.auth_failed,
                                    Toast.LENGTH_SHORT).show();
                        }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.rl_fb_login:
                ...
                break;
            case R.id.b_login:
                signInEmail(userEmail.getText().toString(), userPassword.getText().toString());
                break;
            default:
                return;
        }
    ...
    public void onBackPressed() {
        startActivity(new Intent(UserSignin.this,UserEmailSignup.class));
        finish();
    }
}
