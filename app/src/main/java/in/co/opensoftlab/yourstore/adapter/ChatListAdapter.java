...
public class ChatListAdapter extends RecyclerView
        .Adapter<ChatListAdapter
        .DataObjectHolder> {
    private List<ChatItem> chatItems = new ArrayList<>();
    private static ChatListAdapter.MyClickListener myClickListener;
...
//            pic.setOnClickListener(this);
    ...
    public void onBindViewHolder(DataObjectHolder holder, int position) {
        holder.name.setText(chatItems.get(position).getSeller());
        holder.title.setText(chatItems.get(position).getStoreName());
        Picasso.with(context)
                .load(chatItems.get(position).getSellerUrl())
                .into(holder.pic);
